...
def connect():
    return psycopg2.connect(
        host="ep-lively-silence-351381.eu-central-1.aws.neon.tech",
        database="pandino",
        user="tulas75",
        password="hNOYzc7kiXW8"
    )
...
def add_user(username, api_key, date_valid_until='2024-12-31'):
    ...
    try:
        cursor.execute('INSERT INTO users (username, api_key, date_valid_until) VALUES (%s, %s, %s)', (username, encrypted_api_key.decode(), date_valid_until))
        conn.commit()
    ...
    if users:
        ...
        for user, api_key, date_valid_until in users:
            try:
                ...
                print(f"Username: {user}, ApiKey: {decrypted_api_key},  Date Valid Until: {date_valid_until}")
            except InvalidToken:
                print(f"Username: {user}, ApiKey: decryption failed,  Date Valid Until: {date_valid_until}")
    else:
