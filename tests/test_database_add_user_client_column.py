...
def test_add_user_client_column_uses_fixed_maui_owned_intent(monkeypatch):
    calls = []
    ...
    database_pg.add_user_client_column()

    assert calls == [("maui_schema", "users", "client", "TEXT")]


def test_add_user_client_column_changed_is_success(monkeypatch):
    monkeypatch.setattr(
...
def test_add_user_client_column_unchanged_is_success(monkeypatch):
    monkeypatch.setattr(
...
def test_add_user_client_column_failed_raises(monkeypatch):
    monkeypatch.setattr(
...
def test_add_user_client_column_takes_no_arguments():
    import inspect

    signature = inspect.signature(database_pg.add_user_client_column)
