...
def test_add_usage_service_column_uses_fixed_maui_owned_intent(monkeypatch):
    calls = []
    ...
    database_pg.add_usage_service_column()

    assert calls == [("maui_schema", "logs", "service", "TEXT")]


def test_add_usage_service_column_changed_is_success(monkeypatch):
    monkeypatch.setattr(
...
def test_add_usage_service_column_unchanged_is_success(monkeypatch):
    monkeypatch.setattr(
...
def test_add_usage_service_column_failed_raises(monkeypatch):
    monkeypatch.setattr(
...
def test_add_usage_service_column_takes_no_arguments():
    import inspect

    signature = inspect.signature(database_pg.add_usage_service_column)
