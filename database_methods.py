...
def build_create_users_associazioni_table_query() -> Tuple[sql.Composed, Tuple[Any, ...]]:
    """
    Builds the SQL statement to create the 'users_associazioni' table
    if it does not already exist.

    The table stores farmer login credentials and associates each user
    with an association via a foreign key.

    :return: A tuple containing the composed SQL query and an empty params tuple.
    """
    query = sql.SQL("""
        CREATE TABLE IF NOT EXISTS {table} (
            ...
            CONSTRAINT fk_ass_user
                ...
                REFERENCES {assoc_table}(id)
                ON DELETE CASCADE
        );
    """).format(
        table=sql.Identifier("users_associazioni"),
        assoc_table=sql.Identifier("associazioni")
    )

    return query, ()


def build_get_farmer_by_username_query(username: str) -> Tuple[sql.Composed, Tuple[Any, ...]]:
    """
    Builds a SQL query that retrieves a single farmer (user of an association)
    based on the provided username.

    This query selects the user's ID, username, hashed password, and the
    associated association ID from the 'users_associazioni' table.

    :param username: The username to search for (case-sensitive match).
    :return: A tuple containing the composed SQL query and its parameters.
    """
    query = sql.SQL("""
        SELECT
            {id_col},
    ...
    """).format(
        ...
        username_col=sql.Identifier("username"),
        password_col=sql.Identifier("password_hash"),
        assoc_id_col=sql.Identifier("id_associazione"),
        table=sql.Identifier("users_associazioni"),
    )
...
def build_get_association_by_id_query(assoc_id: int) -> Tuple[sql.Composed, Tuple[Any, ...]]:
    """
    Builds a SQL query that retrieves all available details for a specific association
    identified by its unique ID.

    Explicit column selection is used instead of SELECT * to guarantee a stable
    and predictable output schema, independent of future schema changes.

    :param assoc_id: The ID of the association to retrieve.
    :return: A tuple (SQL query, parameters) ready for psycopg execution.
    """
    query = sql.SQL("""
        SELECT
            {id_col},
    ...
    """).format(
        table=sql.Identifier("associazioni"),
        id_col=sql.Identifier("id"),
        name_col=sql.Identifier("nome_associazione"),
        maschi_col=sql.Identifier("soci_maschi"),
        femmine_col=sql.Identifier("soci_femmine"),
        descr_col=sql.Identifier("descrizione"),
        dist_col=sql.Identifier("distretto"),
        area_col=sql.Identifier("area_coltivata_ha"),
        irrigazione_col=sql.Identifier("sistema_irrigazione"),
        conservazione_col=sql.Identifier("sistema_conservazione"),
        processamento_col=sql.Identifier("sistema_processamento"),
        contatto_col=sql.Identifier("contatto_telefonico"),
    )
...
def build_update_association_query(
    assoc_id: int,
    data: dict
) -> Tuple[sql.Composed, Tuple[Any, ...]]:
    """
    Builds a parameterized SQL UPDATE query for the 'associazioni' table.

    Only known and allowed columns can be updated, ensuring schema safety and
    preventing SQL injection. Columns not belonging to the allowed set are ignored.

    :param assoc_id: ID of the association to update.
    :param data: Dictionary mapping column names to new values.
    :return: (SQL query, parameters) ready for psycopg execution.
    """

    # Columns allowed to be updated
    allowed_columns = {
        "nome_associazione",
        "soci_maschi",
        "soci_femmine",
    ...
    if not updates:
        # Return a valid Composed object
        empty_query = sql.SQL("SELECT 0").format()
        return empty_query, ()
    ...
    """).format(
        table=sql.Identifier("associazioni"),
        assignments=sql.SQL(", ").join(updates),
        id_col=sql.Identifier("id"),
    )

    # Add the association ID at the end of the params
    params.append(assoc_id)

    return query, tuple(params)


def build_get_products_by_association_query(
    assoc_id: int
) -> Tuple[sql.Composed, Tuple[Any, ...]]:
    """
    Builds a SQL query that retrieves all product entries belonging to a specific
    association, identified by its ID.

    Explicit column selection is preferred over SELECT * to provide a stable and
    predictable output schema, independent of future database changes.

    :param assoc_id: The association ID whose products should be retrieved.
    :return: (SQL query, parameters) ready for psycopg execution.
    """
    query = sql.SQL("""
        SELECT
        ...
        FROM {table}
        WHERE {assoc_col} = %s
        ORDER BY {cultura_col} ASC
    """).format(
        table=sql.Identifier("prodotti"),
        id_col=sql.Identifier("id"),
        cultura_col=sql.Identifier("cultura"),
        rendimento_col=sql.Identifier("rendimento_estimado_kg"),
        preco_col=sql.Identifier("preco_venda_estimado_kg"),
        assoc_col=sql.Identifier("id_associazione"),
    )
...
def build_add_product_query(
    data: dict
) -> Tuple[sql.Composed, Tuple[Any, ...]]:
    """
    Builds a parameterized SQL INSERT query for the 'prodotti' table.

    Only expected and allowed columns are inserted. Missing or extra fields
    in the input dictionary are ignored gracefully. The query returns the
    ID of the inserted row.

    :param data: Dictionary containing product fields.
    :return: (SQL query, parameters) ready for psycopg execution.
    """

    # Define allowed columns in stable order
    allowed_columns = [
        "cultura",
        "rendimento_estimado_kg",
        "preco_venda_estimado_kg",
        "id_associazione",
    ]

    # Build the column and placeholder lists
    column_identifiers = [sql.Identifier(c) for c in allowed_columns]
    placeholders = [sql.Placeholder() for _ in allowed_columns]
    ...
    params = tuple(data.get(c) for c in allowed_columns)

    return query, params


def build_update_product_query(
    product_id: int,
    data: dict
) -> Tuple[sql.Composed, Tuple[Any, ...]]:
    """
    Builds a parameterized SQL UPDATE query for the 'prodotti' table.

    Only allowed columns may be updated, ensuring schema consistency and
    preventing SQL injection. Any fields not belonging to the allowed set
    ...
    # No valid fields provided → return a harmless no-op query
    if not updates:
        empty_query = sql.SQL("SELECT 0").format()  # Composed SQL for type safety
        return empty_query, ()
    ...
    # Append product_id at the end of params
    params.append(product_id)

    return query, tuple(params)


def build_delete_product_query(
    product_id: int
) -> Tuple[sql.Composed, Tuple[Any, ...]]:
    """
    Builds a SQL DELETE statement for removing a single product
    identified by its unique ID.

    The query deletes exactly one row in the 'prodotti' table and is fully
    parameterized to ensure SQL injection safety.
...
def build_get_all_farmers_query() -> Tuple[sql.Composed, Tuple[Any, ...]]:
    """
    Builds a SQL query that returns all registered farmers along with the
    name and district of their associated association, if present.

    A LEFT JOIN is used so that users_associazioni entries without a matching
    association record are still included in the result set.

    :return: (SQL query, empty parameter tuple).
    """
    query = sql.SQL("""
        SELECT
            u.{user_id},
        ...
        LEFT JOIN {assoc_table} a
            ON u.{assoc_id} = a.{assoc_pk}
        ORDER BY u.{user_id}
    """).format(
        users_table=sql.Identifier("users_associazioni"),
        assoc_table=sql.Identifier("associazioni"),

        user_id=sql.Identifier("id"),
        username=sql.Identifier("username"),
        assoc_id=sql.Identifier("id_associazione"),

        assoc_pk=sql.Identifier("id"),
        assoc_name=sql.Identifier("nome_associazione"),
        district=sql.Identifier("distretto"),
    )
